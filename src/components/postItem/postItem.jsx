...
const PostItem = ({ props, currentUser }) => {
  ...
  const handleSavePost = async (postId) => {
    try {
      ...
      dispatch(savePost(response.data?.postId))
    } catch (error) {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <div>
            <p className=" font-extrabold font-nunito text-lg text-start">
              {props.username}
            </p>
      ...
      <div className="flex justify-between p-3">
        ...
        <button onClick={() => {
          ...
          <GoBookmark size={25} color={user?.savePost?.includes(props?._id) ? 'orange' : 'black'} />
          {user?.savePost?.includes(props?._id) ? 'Saved' : 'Save'}
        </button>
      </div>
      <div className=" px-4">
